...
export const MovieList = (props: IMovieListProps) => {
    ...
    const fetchCatalogue = useCallback(async (id:number) => {
      let ax = getAxios();
      const response = await ax.get<IMovieCatalogueItem[]>(devApi + `compare/catalogue/${id}`)
...
<TableContainer component={Paper}>
      <Table sx={{ minWidth: 650 }} aria-label="movie price comparison list">
        <TableHead>
          ...
          {moviesList.length > 0 ? moviesList.map((movie:IMovieCatalogueItem) => (
            <MovieListItem
              movieCatalogueItem={movie}
              movieSources={sources}></MovieListItem>
          )): <CircularProgress/>}
      </Table>
    </TableContainer>
    )
